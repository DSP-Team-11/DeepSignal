...
const doppler_downsampling_Tool = {
    ...
    init: function() {
        ...
        // Test server connection on init
        this.testConnection();
    },

    ensureElements: function() {
        const requiredElements = [
            'processingInfo', 'downloadBtn', 'fileInput', 
            'slider', 'srVal', 'player', 'waveCanvas', 'result'
        ];
    ...
    setupEventListeners: function() {
        ...
        if (fileInput) {
            ...
            console.error('❌ fileInput element not found for event listener');
        }
    },
    ...
    testConnection: async function() {
        try {
            ...
            if (response.ok) {
                ...
                this.updateResult("Server connection issue ⚠️", "error");
                return false;
            }
        } catch (error) {
            console.error('❌ Server connection failed:', error);
            this.updateResult("Cannot connect to server ❌", "error");
            return false;
        }
    },
    ...
    uploadFile: async function() {
        ...
        try {
            ...
            const res = await fetch('/doppler-downsampling-upload', { 
                method: 'POST', 
                body: formData 
            });
            ...
            if (!res.ok) {
                // Clone the response before reading it to avoid "body consumed" error
            ...
            // Automatically process with current sample rate
            setTimeout(() => {
            ...
            if (error.name === 'TypeError' && error.message.includes('fetch')) {
                errorMessage += "Network error - check if server is running on http://127.0.0.1:5000";
            } else {
    ...
    updateSR: async function(sr) {
        ...
        try {
            ...
            console.log('📨 Sending downsampling request:', requestBody);
            
            const res = await fetch('/doppler-downsampling', {
                method: 'POST',
                headers: { 
            ...
            console.log('📨 Downsampling response status:', res.status, res.statusText);
            ...
            if (!res.ok) {
                ...
                throw new Error(`Processing failed (${res.status}): ${errorText}`);
            }

            const data = await res.json();
            console.log('✅ Downsampling successful:', data);
            ...
            if (player) {
                ...
                console.log('🎵 Audio player source updated');
            }
            
            this.updateResult(`✅ Downsampled to ${data.sample_rate} Hz`, "success");
            this.processedUrl = data.audio_url;
    ...
    downloadAudio: function() {
        ...
        const downloadUrl = `/doppler-downsampling-download/${filename}`;
    ...
    drawWaveform: async function(url) {
        try {
            ...
            console.log(`📈 Fetching waveform data for: ${filename}`);
            
            const response = await fetch(`/doppler-downsampling-waveform/${filename}`);
            ...
            if (!response.ok) {
                ...
                throw new Error(`Waveform fetch failed: ${errorText}`);
            }
            ...
            // Draw waveform
            this.drawWaveformOnCanvas(ctx, canvas, waveform, sr);

        } catch (error) {
            console.error("❌ Waveform drawing failed:", error);
            this.drawErrorOnCanvas("Unable to draw waveform: " + error.message);
        }
    },

    drawWaveformOnCanvas: function(ctx, canvas, waveform, sampleRate) {
        ...
        for (let i = 0; i < width; i++) {
            const index = Math.min(Math.floor(i * step), len - 1);
        ...
        ctx.fillStyle = "#333";
        ctx.font = "14px Arial";
        ctx.fillText(`Waveform at ${sampleRate} Hz | Max amplitude: ${maxVal.toFixed(3)}`, 10, 20);
...
async function generateDopplerSound() {
    ...
    try {
        ...
        if (!response.ok) {
            // Clone response to avoid body consumption error
            const responseClone = response.clone();
...
// Initialize downsampling tool when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
    doppler_downsampling_Tool.init();
});
