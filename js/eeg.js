...
document.addEventListener('DOMContentLoaded', function() {
   ...
   function initViewer() {
    ...
    function addDownSamplingControls() {
        ...
        downSamplingRow.innerHTML = `
            ...
            <div class="col-md-6">
                <div class="control-group">
                    ...
                    <button id="analyzeDownsampledBtn" class="btn btn-outline-primary w-100 mb-2">
                        <i class="bi-graph-down me-2"></i>Analyze Downsampled
                    </button>
                    <button id="compareAnalysisBtn" class="btn btn-outline-info w-100">
                        <i class="bi-arrow-left-right me-2"></i>Compare Both
                    </button>
        ...
        // Add analyze downsampled button event
        document.getElementById('analyzeDownsampledBtn').addEventListener('click', analyzeDownsampled);
        
        // Add compare analysis button event
        document.getElementById('compareAnalysisBtn').addEventListener('click', compareAnalysis);
    }
    ...
    // Analyze downsampled signal - FIXED VERSION
    async function analyzeDownsampled() {
        const file = signalUpload.files[0];
        
        if (!file) {
            alert('Please upload an EEG signal file first!');
            return;
        }

        const analyzeBtn = document.getElementById('analyzeDownsampledBtn');
        const originalText = analyzeBtn.innerHTML;
        
        analyzeBtn.disabled = true;
        analyzeBtn.innerHTML = '<i class="bi-hourglass-split me-2"></i>Analyzing Downsampled...';

        try {
            const formData = new FormData();
            formData.append('file', file);
            formData.append('downsampling_rate', viewerState.downSamplingRate);

            console.log(`🔧 Sending downsampled analysis request: ${viewerState.downSamplingRate}Hz`);
            
            const response = await fetch('http://127.0.0.1:5000/api/classify_eeg_downsampled', {
                method: 'POST',
                body: formData
            });
            ...
            console.log('📊 Downsampled analysis response:', result);

            if (result.error) {
                throw new Error(result.error || "Unknown error from backend");
            }

            // Show the downsampled classification results
            showDownsampledClassificationResult(result);
            
        } catch (error) {
            console.error('Downsampled analysis error:', error);
            alert('Downsampled analysis failed: ' + error.message);
        } finally {
    ...
    // Compare original vs downsampled analysis - FIXED VERSION
    async function compareAnalysis() {
        ...
        compareBtn.innerHTML = '<i class="bi-hourglass-split me-2"></i>Comparing...';
        ...
        try {
            ...
            console.log(`🔄 Sending comparison request: Original vs ${viewerState.downSamplingRate}Hz`);
            
            const response = await fetch('http://127.0.0.1:5000/api/compare_eeg_analysis', {
                method: 'POST',
                body: formData
            });
            ...
            const result = await response.json();
            console.log('📊 Comparison analysis response:', result);
            ...
            alert('Comparison analysis failed: ' + error.message);
        } finally {
    ...
    // Show downsampled classification results
    function showDownsampledClassificationResult(result) {
                ...
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">
                            <i class="bi-graph-down me-2"></i>
                            Downsampled EEG Analysis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="alert alert-info">
                            <strong>Sampling Rate:</strong> ${result.downsampling_rate} Hz (Original: 128 Hz)
                        </div>
                        <div class="text-center mb-4">
                            <h3 class="text-warning">${result.prediction}</h3>
                            <div class="confidence-badge badge bg-warning fs-6 mt-2">
                        ...
                        <div class="probability-bars">
                            ${Object.entries(result.probabilities).map(([condition, prob]) => `
                                <div class="d-flex align-items-center mb-2">
                                    <div class="condition-label me-2" style="width: 200px;">${condition}</div>
                                    <div class="progress flex-grow-1 me-2" style="height: 20px;">
                                        <div class="progress-bar ${condition === result.prediction ? 'bg-warning' : 'bg-light text-dark'}" 
                                             role="progressbar" 
                        ...
                        <div class="mt-3 small text-muted">
                            ...
                            Signal shape after downsampling: [${result.signal_shape_after_downsampling.join(', ')}]
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            ...
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-header bg-primary text-white">
                                        <h6 class="mb-0">
                                            <i class="bi-play-circle me-2"></i>
                                            Original Signal (128 Hz)
                                        </h6>
                                    </div>
                                    <div class="card-body text-center">
                                        <h4 class="text-primary">${result.original_analysis.prediction}</h4>
                                        <div class="confidence-badge badge bg-primary fs-5 mt-2">
                                            ${(result.original_analysis.confidence * 100).toFixed(1)}% Confidence
                                        </div>
                                        <div class="mt-3">
                                            <small class="text-muted">Signal shape: [${result.original_analysis.signal_shape.join(', ')}]</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-header bg-warning text-dark">
                                        <h6 class="mb-0">
                                            <i class="bi-graph-down me-2"></i>
                                            Downsampled (${result.downsampled_analysis.downsampling_rate} Hz)
                                        </h6>
                                    </div>
                                    <div class="card-body text-center">
                                        <h4 class="text-warning">${result.downsampled_analysis.prediction}</h4>
                                        <div class="confidence-badge badge bg-warning fs-5 mt-2">
                                            ${(result.downsampled_analysis.confidence * 100).toFixed(1)}% Confidence
                                        </div>
                                        <div class="mt-3">
                                            <small class="text-muted">Signal shape: [${result.downsampled_analysis.signal_shape.join(', ')}]</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row mt-4">
                            <div class="col-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ${result.comparison.prediction_changed ? 
                                            `<div class="alert alert-warning">
                                                <i class="bi-exclamation-triangle me-2"></i>
                                                <strong>Prediction Changed:</strong> Downsampling affected the model's diagnosis
                                            </div>` :
                                            `<div class="alert alert-success">
                                                <i class="bi-check-circle me-2"></i>
                                                <strong>Prediction Consistent:</strong> Downsampling did not affect the diagnosis
                                            </div>`
                                        }
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <span class="badge ${result.comparison.confidence_difference > 0 ? 'bg-success' : 'bg-danger'}">
                                                    ${result.comparison.confidence_difference > 0 ? '+' : ''}${(result.comparison.confidence_difference * 100).toFixed(2)}%
                                                </span>
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Sampling Rates:</strong>
                                                <span class="badge bg-primary">Original: ${result.comparison.original_rate}Hz</span>
                                                <span class="badge bg-warning text-dark">Downsampled: ${result.comparison.downsampled_rate}Hz</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="row mt-4">
                            <div class="col-12">
                                <h6>Probability Distribution Comparison</h6>
                                <div class="table-responsive">
                                    <table class="table table-sm table-bordered">
                                        <thead class="table-light">
                                            <tr>
                                                <th>Condition</th>
                                                <th>Original (128Hz)</th>
                                                <th>Downsampled (${result.downsampled_analysis.downsampling_rate}Hz)</th>
                                                <th>Difference</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ${Object.keys(result.original_analysis.probabilities).map(condition => {
                                                const origProb = result.original_analysis.probabilities[condition];
                                                const downProb = result.downsampled_analysis.probabilities[condition];
                                                const diff = downProb - origProb;
                                                const isPrediction = condition === result.original_analysis.prediction;
                                                return `
                                                    <tr class="${isPrediction ? 'table-active' : ''}">
                                                        <td><strong>${condition}</strong></td>
                                                        <td>${(origProb * 100).toFixed(1)}%</td>
                                                        <td>${(downProb * 100).toFixed(1)}%</td>
                                                        <td>
